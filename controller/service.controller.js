...
controller.serviceCreate = async (req,res)=>{
	try {
		...
		req.body.imgUrl = await uploadFile.uploadSingle(req.file, {folder:"service"})
...
controller.updateService= async (req,res) =>{
	try{
		...
		if(req.file){
			...
			let deleteService= await serviceModel.findOne({ _id: id })
			console.log(`deleteService`, deleteService)
			if(deleteService) await uploadFile.deleteSingle(deleteService.img)
			req.body.img = await uploadFile.uploadSingle(req.file, {folder:"service"})
...
controller.deleteService= async (req,res) =>{
	try{
		...
		if(service) await uploadFile.deleteSingle(service.img)
