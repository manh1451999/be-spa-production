...
controller.deleteProductById = async (req, res) => {
	...
	try {
		...
		if(product) await uploadFile.deleteSingle(product.imgUrl)
...
controller.updateProduct = async (req, res) => {
	...
	try {
		...
		if (req.file) {
			let deleteProduct= await productModel.findById(id)
			if(deleteProduct) await uploadFile.deleteSingle(deleteProduct.imgUrl)
			req.body.imgUrl = await uploadFile.uploadSingle(req.file, {folder:"product"})
			// req.body.imgUrl = '/' + req.file.path.split('\\').slice(1).join('/');
		}
		// else if (req.body.imgUrl) delete req.body.imgUrl
...
controller.uploadImg = async (req, res) => {
	...
	try {
		...
		if (req.file) {
			...
			fileUpload = await uploadFile.uploadSingle(req.file, {folder:"product"})
			deleteFile = await uploadFile.deleteSingle(fileUpload)
		}
